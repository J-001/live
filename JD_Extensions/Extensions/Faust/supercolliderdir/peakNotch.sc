FaustPeakNotch : UGen
{
  *ar { | in1, freq(1000.0), gain(1.0), q(1.0) |
      ^this.multiNew('audio', in1, freq, gain, q)
  }

  *kr { | in1, freq(1000.0), gain(1.0), q(1.0) |
...
//    SynthDef.writeOnce(\faustPeakNotch,
      SynthDef(\faustPeakNotch,
        { |out=0|
          Out.ar(out, 
            FaustPeakNotch.ar(
              \in1.ar(0), \freq.kr(1000.0), \gain.kr(1.0), \q.kr(1.0)
            )
          )
        }, metadata: (specs:( 
        //\controlName:[min, max, warp, step, default, units].asSpec,
          \freq:[100.0, 10000.0, 0, 1.0, 1000.0].asSpec,
          \gain:[0.0, 8.0, 0, 0.01, 1.0].asSpec,
